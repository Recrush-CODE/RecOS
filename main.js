...
function executeCommand(command, output, windowId) {
    ...
    if (cmd === 'date' || cmd === 'Date' || cmd === 'DATE') {
        ...
        const gmtOffset = date.toString().match(/GMT[+-]\d+/)[0];
        const timeZoneFormatter = new Intl.DateTimeFormat('en', { timeZoneName: 'long' });
        const timeZoneName = timeZoneFormatter.formatToParts(date).find(part => part.type === 'timeZoneName').value;
        response.textContent = `${weekday} ${month} ${day} ${year} ${hours}:${minutes}:${seconds} ${gmtOffset} (${timeZoneName})`;
